...
For copy/paste examples, see [`.env.advanced.example` on GitHub](https://github.com/forkluck/forkluck-azure-provider/blob/main/.env.advanced.example).

## Retry and backoff tuning

| Variable | Default | What it controls | When to change |
|----------|---------|------------------|----------------|
| `MAIL_SEND_MAX_RETRIES` (alias `SES_SEND_MAX_RETRIES`) | `3` | Retries per recipient when the provider returns transient/throttling errors. | Increase for unstable AWS/network conditions; decrease for faster fail-fast behavior. |
| `MAIL_RETRY_BASE_MS` (alias `SES_RETRY_BASE_MS`) | `500` | Base backoff (milliseconds) before retry jitter is applied. | Increase to reduce pressure during throttling bursts. |
| `MAIL_RETRY_MAX_MS` (alias `SES_RETRY_MAX_MS`) | `10000` | Maximum backoff cap (milliseconds). | Increase if provider throttling persists and you want slower retry pacing. |
...
| Variable | Default | What it controls | When to change |
|----------|---------|------------------|----------------|
| `DISABLE_MAIL_EVENT_POLLER` (alias `DISABLE_SES_EVENT_POLLER`) | `false` | Disables the mail event queue worker loop. | Enable (`1`) for local dashboard styling/demo work without event traffic. |
