...
For retry/backoff tuning, request-size limits, Ghost Admin API compatibility overrides, and local dev/testing switches, see [Advanced configuration](./advanced-config.html) and [`.env.advanced.example` on GitHub](https://github.com/forkluck/forkluck-azure-provider/blob/main/.env.advanced.example).
