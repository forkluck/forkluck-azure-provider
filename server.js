...
async function bootstrap() {
  ...
  if (isApiRole()) {
    ...
    server = app.listen(config.port, function() {
      ...
      console.log('  Mail provider: ' + described.provider);
      console.log('  ' + (described.region ? 'Region: ' + described.region : 'Endpoint: ' + described.endpoint));
      ...
      console.log('  Database: ' + (config.database.driver === 'sqlite' ? 'SQLite ' + config.database.file : 'MySQL'));
      console.log('  Send queue: ' + described.sendQueue);
      console.log('  Event queue: ' + described.eventsQueue);
  ...
  if (isWorkerRole()) {
    ...
    if (config.disableEventPoller) {
      console.log('Mail event poller: DISABLED (DISABLE_MAIL_EVENT_POLLER=1)');
    } else {
