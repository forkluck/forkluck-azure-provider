(function () {
  ...
  function isAzureProvider() {
    return String(runtimeConfig.mailProvider || 'ses').toLowerCase() === 'azure';
  }

  function providerLabel() {
    return isAzureProvider() ? 'ACS' : 'SES';
  }

  function providerLocation() {
    if (isAzureProvider()) {
      return String(runtimeConfig.providerEndpoint || 'this Communication Services resource');
    }
    return String(runtimeConfig.awsRegion || 'this region');
  }
  ...
  function formatSesAccountError(sesAccount) {
    ...
    if (isAzureProvider()) {
      return message ? 'Provider status unavailable: ' + message : 'Provider status unavailable.';
    }
  ...
  function humanizeFailureReason(row) {
    ...
    if (isAzureProvider()) {
      if (lower.indexOf('429') !== -1 || lower.indexOf('too many requests') !== -1) {
        return 'Azure Communication Services rate limit reached';
      }
      if (lower.indexOf('senderaddress') !== -1 || lower.indexOf('sender address') !== -1) {
        return 'Sender address is not allowed for this resource';
      }
      if (lower.indexOf('domain') !== -1) {
        return 'Sender domain is not linked to the Communication Services resource';
      }
    }
  ...
  function renderOverview() {
    ...
    if (!isAzureProvider() && sesAccount.available && !sesAccount.productionAccessEnabled) {
      alerts.push({
        level: 'warn',
        text: 'SES sandbox active in ' + providerLocation() + '. Only verified recipients can receive mail.'
      });
    }
    if (sesAccount.available && sesAccount.sendingEnabled === false) {
      alerts.push({ level: 'danger', text: providerLabel() + ' sending is disabled for this account.' });
    }
  ...
  function renderHealth() {
    ...
    var rows = [
      ['Mail provider', providerLabel()],
      [providerLabel() + ' account mode', sesModeLabel(sesAccount)],
      [providerLabel() + ' max send rate', formatQuotaValue(sesAccount.sendQuota.maxSendRate, '/sec')],
      [providerLabel() + ' 24h quota', formatQuotaValue(sesAccount.sendQuota.max24HourSend, '')],
      ['Newsletter send queue', sendQueueDepth === null ? '-' : String(sendQueueDepth)],
      ['Mail event queue', queueDepth === null ? '-' : String(queueDepth)],
    ...
    if (sesAccount.available) {
      rows.splice(2, 0, [providerLabel() + ' sending enabled', sesAccount.sendingEnabled ? 'Yes' : 'No']);
      if (!isAzureProvider()) {
        rows.splice(3, 0, ['SES enforcement status', sesAccount.enforcementStatus || '-']);
      }
      rows.push([providerLabel() + ' account check', sesAccount.checkedAt ? fmtIso(sesAccount.checkedAt) : '-']);
    }
    ...
    if (sesAccount.error) {
      rows.push([providerLabel() + ' account status error', sesAccount.error]);
    }
