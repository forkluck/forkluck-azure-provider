...
class BatchSendingService {
      ...
      await self.emailProvider.send(batch, recipientBatch, {
        onSuccess: async function(recipient, outcome) {
          ...
          if (config.logLevel === 'debug') {
            ...
            console.log('Sent to ' + recipient + ' (provider message ID: ' + outcome.result.messageId + ')' + retriesUsed);
          }
        },
  ...
  async pollOnce(workerInstanceId) {
    var queue = getProvider().queue;
    var messages = await queue.receiveMessages(
    ...
    for (var i = 0; i < messages.length; i += 1) {
      ...
      try {
        ...
        await queue.deleteMessage(config.newsletterSendQueueUrl, message.receiptHandle);
        continue;
      }
