// SES/SNS event → Mailgun event normalization.
...
function mapEvent(sesEvent) {
  ...
  var providerMessageId = sesEvent.mail && sesEvent.mail.messageId || null;
...
// SES events arrive either wrapped in an SNS notification envelope or raw.
function parseQueueMessage(body) {
  var parsed = JSON.parse(body);

  if (parsed.Type === 'Notification' && parsed.Message) {
    return {
      events: [JSON.parse(parsed.Message)],
...
function describeEvent(sesEvent) {
  return (sesEvent && sesEvent.eventType) || 'unknown';
}
