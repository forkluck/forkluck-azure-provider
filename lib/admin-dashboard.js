...
var { query, getDialect, getTableCounts, listRuntimeHeartbeats } = require('./db');
...
async function getSummary(periodKey) {
  ...
  var suppressionRows = await query(
    'SELECT type, COUNT(*) AS c FROM suppressions WHERE created_at >= ' + getDialect().fromUnixtime('?') + ' GROUP BY type',
    [since]
  );
...
async function getSuppressions(limit, periodKey) {
  ...
  if (periodKey) {
    sql += ' WHERE created_at >= ' + getDialect().fromUnixtime('?');
    params.push(getPeriodWindow(periodKey).startSeconds);
  }
...
async function getQueueSnapshot() {
  ...
  try {
    sendQueue = await getProvider().queue.getQueueDepth(config.newsletterSendQueueUrl);
  } catch (err) {
    sendQueueError = err && err.message ? err.message : String(err);
  }

  try {
    eventsQueue = await getProvider().queue.getQueueDepth(config.sesEventsQueueUrl);
  } catch (err) {
...
function buildRuntimeConfig(basePath) {
  return {
    ...
    providerEndpoint: getProvider().describe().endpoint || ''
  };
}
...
function createAdminRouter(getRuntimeStatus) {
  ...
  router.get('/api/health', withAsync(async function(req, res) {
    ...
    var sesAccount = await getProvider().account.getAccountStatus();
    var health = {
      status: 'ok',
  ...
  router.use(function(err, _req, res, _next) {
    ...
    if (status >= 500) console.error('Admin dashboard error:', detail);
