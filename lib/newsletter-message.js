...
function parseFormData(req) {
  return new Promise(function(resolve, reject) {
    ...
    // 'aborted' is deprecated on IncomingMessage; an aborted request emits
    // 'error' (once a listener exists) and 'close' with `complete` false.
    req.on('error', function() {
      fail(createRequestError('Request aborted', 400));
    });
    req.on('close', function() {
      if (!req.complete) fail(createRequestError('Request aborted', 400));
    });
